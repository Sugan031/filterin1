...
const ReferenceMapTable = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setTotalValues(response.data.data.total);
  ...
  const fetchMoreData = async()=>{
    setHasMore(true);
    setPageApi(pageApi+1);
     await fetchData(pageApi);
  }
// console.log("******************88")
console.log(Array.isArray(columnNames));
...
// console.log("***************888")
  ...
  return (
    <Grid item xs={8} height={650} mt={3} style={{overflow:'scroll'}}>
      {/* <InfiniteScroll
        dataLength={totalValues}
        next={fetchMoreData}
        hasMore={hasMore}
        loader={<p>loading...</p>}
        endMessage={'All the values are loaded'}
        // onScroll={(e, value) => setPageApi(value)}
      > */}
    <DataGrid
      sx={{
...
export default ReferenceMapTable
