...
export default function Reference() {
  ...
  const fetchData = async () => {
    try {
      ...
      setBaseData(response.data.baseData);
      let columnName = response.data.columns
      
      let temp = columnName.map((item) =>{
      let name = ''
        if(item === 'name'){
          name = "Hotel Name"
        }else{
          let headerName =  item.split('_')
          name = headerName[0][0].toUpperCase() + headerName[0].slice(1,)
  
          if(headerName[1]){
            name += ' ' + headerName[1][0].toUpperCase() + headerName[1].slice(1,)
          }
        }
      ...
      console.log(temp)
      setColumnsNames(temp)
    // }
     
      // setTotalPages(response.data.count);
      // setColumnsNames(response.data.columns);
    } catch (error) {
    ...
    >
      ...
      >
        <Grid item xs={6} width="50%" mt={3}>
          ...
          >
            ...
            >
              ...
              <TableBody>
              {baseData && (
        <TableRow hover role="checkbox" tabIndex={-1} key={baseData.code}>
          {HotelSearchColumn.map((column) => {
            const value = baseData[column.field]; // Access data using column.field
            return (
              <TableCell key={column.field} align={column.align}>
                ...
                {/* <TableRow key={(baseData) => baseData.ind}> */}
        ...
        <Grid item xs={8} height={500} mt={3}>
          <DataGrid
            getRowId={(mappedData) => mappedData.ind}
            columns={columnNames}
            rows={mappedData?.map((mappedData) => ({ ...mappedData, ind: mappedData.ind + 1 }))}
            pageSize={10}
            checkboxSelection
